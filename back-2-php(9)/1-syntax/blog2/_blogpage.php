...
$sqlall_blog = "SELECT * FROM `azuma_php_blog1` where delflag = '0' and blogid = 'blog2'"; // ★ 블로그 ID
$count_blog = mysqli_query($conn, $sqlall_blog)->num_rows;
$end_blog_page = ceil($count_blog/3);
// ▲ ★ 넘기는 개수 수정할 때 맨 끝 숫자 수정
$sql_blog = "SELECT * FROM `azuma_php_blog1` where delflag = '0' and blogid = 'blog2' order by no desc limit $start_blog, 3"; // ★ 블로그 ID
// ▲ ★ 넘기는 개수 수정할 때 맨 끝 숫자 수정
