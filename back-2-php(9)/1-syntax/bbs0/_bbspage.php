...
$sqlall_bbs = "SELECT * FROM `azuma_php_bbs1` where delflag = '0' and bbsid = 'bbs0'"; // ★ 게시물 ID
$count_bbs = mysqli_query($conn, $sqlall_bbs)->num_rows;
$end_bbs_page = ceil($count_bbs/3);
// ▲ ★ 넘기는 개수 수정할 때 맨 끝 숫자 수정
$sql_bbs = "SELECT * FROM `azuma_php_bbs1` where delflag = '0' and bbsid = 'bbs0' order by no desc limit $start_bbs, 3"; // ★ 게시물 ID
// ▲ ★ 넘기는 개수 수정할 때 맨 끝 숫자 수정
