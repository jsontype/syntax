  ...
  if($_SESSION['id'] == 'admin'){
        ...
        if(isset($img1)) {
          ...
          if(move_uploaded_file($tmp_name1, $dir.$file_name1)){
            require ('_conn.php');
            $sql = "UPDATE `azuma_f_bbs2` SET `title` = '$title', `content` = '$content', `nickname` = '$nickname', `mod_date` = CURRENT_TIMESTAMP, `blackbox1` = '$blackbox1', `tag1` = '$tag1', `img_file1` = '$file_name1', `img_size1` = '$img_size1' WHERE `azuma_f_bbs2`.`no`=$no";
            mysqli_query($conn, $sql);
          } else {
            // 파일등록처리 : 이미지파일 업로드중 실패했을 경우
            require ('_conn.php');
            $sql = "UPDATE `azuma_f_bbs2` SET `title` = '$title', `content` = '$content', `nickname` = '$nickname', `mod_date` = CURRENT_TIMESTAMP, `blackbox1` = '$blackbox1', `tag1` = '$tag1' WHERE `azuma_f_bbs2`.`no`=$no";
            mysqli_query($conn, $sql);
          }
          } else {
