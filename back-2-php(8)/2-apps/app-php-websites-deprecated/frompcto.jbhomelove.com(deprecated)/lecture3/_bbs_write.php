...
if(isset($img1)) {
    ...
    if(move_uploaded_file($tmp_name1, $dir.$file_name1)){
        // DB에 중복방지 처리된 파일등록
        require ('_conn.php');
        $sql = "INSERT INTO `azuma_f_bbs3` (`no`, `title`, `content`, `nickname`, `reg_date`, `mod_date`, `blackbox1`, `tag1`, `view`, `del_flg`, `img_file1`, `img_size1`)
  ...
  // 파일입력 안했을 시
    ...
    require ('_conn.php');
    $sql = "INSERT INTO `azuma_f_bbs3` (`no`, `title`, `content`, `nickname`, `reg_date`, `mod_date`, `blackbox1`, `tag1`, `view`, `del_flg`, `img_file1`, `img_size1`) VALUES (NULL, '$title', '$content', '$nickname', current_timestamp(), NULL, '$blackbox1', '$tag1', '0', '0', NULL, NULL)";
    mysqli_query($conn, $sql);
    echo "<meta http-equiv='refresh' content='0.1; url=bbs.php'>"; // 일반적인 내림차순의 경우 최신글이 제일 위에 올라오므로 그냥 bbs.php로 바로 가면 된다.
  }
