...
require ('_conn.php');
          ...
          // <!-- 잘못 입력된 경우 걸러내기 -->
          $sql_pwc = "SELECT * FROM `azuma_d_blog2` WHERE no = $no and pw = $pw";
          $pwc = mysqli_query($conn, $sql_pwc)->num_rows;
          if ($pwc < 1) {
            echo "<script>alert('등록시의 비밀번호를 올바르게 입력하세요.');history.back();</script>";
          } else if (!$title||!$content||!$pw||!$nickname) {   // 필수항목 미입력 걸러내기
          ...
          if(isset($img1)) {
            ...
            if(move_uploaded_file($tmp_name1, $dir.$file_name1)){
              require ('_conn.php');
              $sql = "UPDATE `azuma_d_blog2` SET `nickname` = '$nickname', `title` = '$title', `content` = '$content', `imgfile1` = '$file_name1', `imgsize1` = '$imgsize1' WHERE `azuma_d_blog2`.`no`=$no";
              mysqli_query($conn, $sql);
            } else {
              // 파일등록처리 : 이미지파일 업로드중 실패했을 경우
              require ('_conn.php');
              $sql = "UPDATE `azuma_d_blog2` SET `nickname` = '$nickname', `title` = '$title', `content` = '$content' WHERE `azuma_d_blog2`.`no`=$no";
              mysqli_query($conn, $sql);
            }
            } else {
