...
$sqlall_bbs = "SELECT * FROM `azuma_f_bbs2` where del_flg = '0'";
$count_bbs = mysqli_query($conn, $sqlall_bbs)->num_rows;
$end_bbs_page = ceil($count_bbs/5);
// ▲ ★ 넘기는 개수 수정할 때 맨 끝 숫자 수정
// [Toggle] 최신순 ON/OFF
// $sql_bbs = "SELECT * FROM `azuma_f_bbs2` where del_flg = '0' order by no desc limit $start_bbs, 5"; // ★ 게시물 ID
$sql_bbs = "SELECT * FROM `azuma_f_bbs2` where del_flg = '0' limit $start_bbs, 5"; // ★ 게시물 ID
// ▲ ★ 넘기는 개수 수정할 때 맨 끝 숫자 수정
