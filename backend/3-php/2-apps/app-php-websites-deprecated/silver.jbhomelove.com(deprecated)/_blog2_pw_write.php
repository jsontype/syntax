...
if(isset($img1)) {
    ...
    if(move_uploaded_file($tmp_name1, $dir.$file_name1)){
        // DB에 중복방지 처리된 파일등록
        require ('_conn.php');
        $sql = "INSERT INTO `azuma_d_blog2` (`id`, `pw`, `nickname`, `title`, `content`, `regdate`, `delflag`, `imgfile1`, `imgsize1`) VALUES ('$id', '$pw', '$nickname', '$title', '$content', current_timestamp(), 0, '$file_name1', '$imgsize1')";
        mysqli_query($conn, $sql);
        echo "<meta http-equiv='refresh' content='0.1; url=blog2.php'>"; // 일반적인 내림차순의 경우 최신글이 제일 위에 올라오므로 그냥 bbs.php로 바로 가면 된다.
        } else {
        // 파일입력 실패시
        require ('_conn.php');
        $sql = "INSERT INTO `azuma_d_blog2` (`id`, `pw`, `nickname`, `title`, `content`, `regdate`, `delflag`, `imgfile1`, `imgsize1`) VALUES ('$id', '$pw', '$nickname', '$title', '$content', current_timestamp(), 0, null, null)";
        mysqli_query($conn, $sql);
        echo "<meta http-equiv='refresh' content='0.1; url=blog2.php'>"; // 일반적인 내림차순의 경우 최신글이 제일 위에 올라오므로 그냥 bbs.php로 바로 가면 된다.
        }
