/*
    ...
    1. 자세히 설명 : 
        ...
        이런 함수를 고차 함수라고 한다.
        고차 함수에서, 인자로 넘겨지는 함수를 콜백 함수라고 한다. 
        즉, 콜백 함수는, 단지 함수를 등록하기만 하고 어떤 이벤트가 발생했거나 특정 시점에 도달했을 때에만 시스템에서 호출하는 함수이다.
...
// 1. 콜백함수 문법
let value = 1
function a(param) { // a()와 같이, "함수를 param으로 전달받거나, return값으로 반환하는 함수"를 고차함수라 한다.
    return param(value)
}
function print(param) { // print는 a() 고차함수 안에서 호출되는 콜백 함수이다.
    return param + 1
}
console.log('결과: ', a(print)) // 결과 : 2
    ...
    1. 콜백함수가 필요한 이유
        ...
        콜백함수를 설명할 때에는 동기/비동기(synchronous/Asynchronous)처리에 대한 이야기도 하면 좋을 것 같다.
            ...
            비동기 : 어떤 요청이 오면 완료가 되기 전에 다음 요청을 실행하는 방식
                ...
                그러나, 이를 자주 하다보니 콜백 지옥이 발생하여 개발자들이 많은 어려움을 겪었다.
                그래서 그 후, 이를 promise나 async/await를 써서 해결하게 된다.
*/
