...
<div class="container">
  ...
  <!-- DB 호출 (위치 변경 안됨) -->
  <?php
  $sql = "SELECT * FROM `azuma_f_bbs2` WHERE no = $no";
  ...
  <div class="row">
    <ul class="list-group">
      <!-- 이전 강의 타이틀 -->
      <?php
      $sqlminus = "SELECT * FROM `azuma_f_bbs2` WHERE no = $no-1 and del_flg = '0'";
      ...
      <!-- 다음 강의 타이틀 -->
      <?php
      $sqlplus = "SELECT * FROM `azuma_f_bbs2` WHERE no = $no+1 and del_flg = '0'";
...
<?php
// 댓글 수 표시하기
$sql_count = "SELECT * FROM `azuma_f_comment1` WHERE del_flg = '0' AND cid = 'bbs2' AND no = '$no'";
