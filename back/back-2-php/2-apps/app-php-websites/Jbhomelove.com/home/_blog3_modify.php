...
require ('_conn.php');
          ...
          if(isset($img1)) {
            ...
            if(move_uploaded_file($tmp_name1, $dir.$file_name1)){
              require ('_conn.php');
              $sql = "UPDATE `azuma_jb_blog3` SET `nickname` = '$nickname', `title` = '$title', `content` = '$content', `imgfile1` = '$file_name1', `imgsize1` = '$imgsize1' WHERE `azuma_jb_blog3`.`no`=$no";
              mysqli_query($conn, $sql);
            } else {
              // 파일등록처리 : 이미지파일 업로드중 실패했을 경우
              require ('_conn.php');
              $sql = "UPDATE `azuma_jb_blog3` SET `nickname` = '$nickname', `title` = '$title', `content` = '$content' WHERE `azuma_jb_blog3`.`no`=$no";
              mysqli_query($conn, $sql);
            }
            } else {
