...
// <!-- 게시물에 해당하는 코멘트 불러오기 -->
$sql_comment = "SELECT * FROM `azuma_php_comment1` WHERE delflag='0' AND cid='blog2' AND no={$v['no']}"; // ★ 블로그 ID : cid는 blogid와 동일하게 설정한다. cid가 없으면 타게시판/블로그와 no 동일할 경우에 다른 곳의 댓글이 여기서도 댓글이 보이게 된다.
...
<form action="_comment_write.php" method="post">
  ...
  <input type='hidden' name='cid' value='blog2'> <!-- ★ 블로그 ID : cid는 blogid와 동일하게 설정한다. cid가 없으면 타게시판/블로그와 no 동일할 경우에 다른 곳의 댓글이 여기서도 댓글이 보이게 된다. -->
