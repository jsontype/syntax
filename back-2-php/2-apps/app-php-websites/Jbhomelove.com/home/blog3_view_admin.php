            ...
            <div class="tag-widget post-tag-container mb-5 mt-5">
              <div class="tagcloud">
                ...
                if(isset($_SESSION['id'])){
                  if($_SESSION['id'] == $v['id']){ ?>
                    <a href='blog3_admin_modify.php?no=<?=$v['no']?>'>수정</a>
                    <a href="_blog3_admin_delete.php?no=<?=$v['no']?>" class="tag-cloud-link" OnClick="return confirm('관리자 모드에서는 비밀번호를 묻지 않고 바로 삭제합니다. 정말 삭제하시겠습니까?');">삭제</a>
                    <a href='blog3_admin.php' class='tag-cloud-link' class='tag-cloud-link'>목록</a>
                <?php } else { ?>
                  <a href="_blog3_admin_delete.php?no=<?=$v['no']?>" class="tag-cloud-link" OnClick="return confirm('관리자 모드에서는 비밀번호를 묻지 않고 바로 삭제합니다. 정말 삭제하시겠습니까?');">삭제</a>
                  <a href='blog3_admin.php' class='tag-cloud-link' class='tag-cloud-link'>목록</a>
                <?php } } else { ?>
