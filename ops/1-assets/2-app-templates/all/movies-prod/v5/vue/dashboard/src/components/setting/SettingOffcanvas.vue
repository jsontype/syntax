<template>
  ...
  <b-offcanvas id="live-customizer" v-model="show" :backdrop="false" title="" :placement="themeSchemeDirection !== 'ltr' ? 'start' : 'end'" class="live-customizer" :body-scrolling="true">
    <template #title>
      <div class="d-flex align-items-center justify-content-between">Setting Panel</div>
    </template>
    <div class="row">
      <div class="col-lg-12">
        <div>
          ...
          <div>
            ...
            <!-- Color customizer end here -->
            <hr class="hr-horizontal" />
            <!-- Theme Direction start here -->
            <ThemeDirectionSection />
            <!-- Theme Direction end here -->
          </div>
        </div>
      </div>
    </div>
  </b-offcanvas>
  <a id="settingbutton" class="btn btn-fixed-end btn-warning btn-icon btn-setting" role="button" @click="open">
...
// Style Components
import ThemeSchemeSection from './sections/ThemeScheme.vue';
...
import ThemeDirectionSection from './sections/ThemeDirection.vue';
import PageStyle from './sections/PageStyle.vue';
...
export default {
  ...
  setup() {
    ...
    const themeSchemeDirection = computed(() => store.theme_scheme_direction_custom);
