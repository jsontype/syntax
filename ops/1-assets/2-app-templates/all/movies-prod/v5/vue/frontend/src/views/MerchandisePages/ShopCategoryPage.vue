    ...
    <b-container>
      <b-row>
        <b-col lg="3">
          <div class="pe-3">
            <div class="shop-box">
              <h5 class="mb-4 text-uppercase">{{ $t('shop.product_categories') }}</h5>
              <ul class="list-unstyled p-0 m-0 shop-list-checkbox">
                <li>{{ $t('shop.activeman') }}</li>
                <li>
                  <div class="d-flex align-items-center justify-content-between">
                    <span>{{ $t('shop.disney_world') }}</span>
                    <a class="checkbox collapsed" data-bs-toggle="collapse" href="#categoriesone" role="button" aria-expanded="false" aria-controls="categoriesone"> </a>
                  </div>
            ...
            <div class="shop-box border-bottom-0">
              <h5 class="mb-4 text-uppercase">{{ $t('shop.products') }}</h5>
              <ul v-for="data in newProduct" :key="data" class="list-unstyled m-0 p-0 shop-product">
                <ShopCategorySidebar :thumbnail="data.thumbnail" :product-name="data.product_name" :price="data.price" :final-price="data.final_price" :additional-info="data.additional_info" />
              </ul>
            </div>
          </div>
        </b-col>
        <b-col lg="9">
          <div class="d-flex align-items-center justify-content-between mb-5 shop-filter flex-wrap5">
            <p v-if="filterWiseProducts.length > 1">{{ $t('shop.showing') }}</p>
            <p v-else>{{ $t('shop.showing_single') }}</p>
            <div class="d-flex align-items-center">
              <div class="product-view-button">
                <ul id="pills-tab" class="nav_shop nav d-flex nav-pills mb-0 iq-product-filter d-flex bg-transparent align-items-center list-inline" role="tablist">
                  <li class="nav-item" role="presentation">
                    <button id="list-view-tab" class="nav-link btn-sm btn-icon rounded-pill p-0" data-bs-toggle="pill" data-bs-target="#pills-list-view" type="button" role="tab" aria-controls="pills-list-view" aria-selected="true">
                  ...
                  <li class="nav-item" role="presentation">
                    <button id="grid-view-tab" class="nav-link btn-sm btn-icon rounded-pill p-0" data-bs-toggle="pill" data-bs-target="#pills-grid-view" type="button" role="tab" aria-controls="pills-grid-view" aria-selected="false">
                      <span class="btn-inner">
                  ...
                  <li class="nav-item" role="presentation">
                    <button id="grid-three-view-tab" class="nav-link btn-sm btn-icon rounded-pill p-0 active show" data-bs-toggle="pill" data-bs-target="#pills-grid-three-view-tab" type="button" role="tab" aria-controls="pills-grid-three-view-tab" aria-selected="false">
                      <span class="btn-inner">
                        <svg class="hover_effect active_effect" width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <li class="nav-item" role="presentation">
                    <button id="grid-four-view-tab" class="nav-link btn-sm btn-icon rounded-pill p-0" data-bs-toggle="pill" data-bs-target="#pills-grid-four-view-tab" type="button" role="tab" aria-controls="pills-grid-four-view-tab" aria-selected="false">
                      <span class="btn-inner">
                        <svg class="hover_effect active_effect" width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <div class="iq-custom-select d-inline-block">
                <select ref="selectElement" class="form-control movie-select" :tabindex="tabindex">
                  <option value="season1">{{ $t('shop.default_sorting') }}</option>
                  <option value="season2">{{ $t('shop.sort_by_popularity') }}</option>
                  <option value="season3">{{ $t('shop.sort_by_rating') }}</option>
                  <option value="season4">{{ $t('shop.sort_by_latest') }}</option>
                  <option value="season5">{{ $t('shop.price_low_to_high') }}</option>
                  <option value="season5">{{ $t('shop.price_high_to_low') }}</option>
                </select>
              </div>
            </div>
          </div>
          ...
          <div id="pills-tabContent" class="tab-content">
            ...
            <div id="pills-list-view" class="tab-pane fade" role="tabpanel" aria-labelledby="list-view-tab">
              <b-row class="row-cols-1">
                <b-col v-for="data in newProduct" :key="data">
                  <CardShopListView :thumbnail="data.thumbnail" :is-sale="data.is_sale" :is-new="data.is_new" :product-name="data.product_name" :price="data.price" :final-price="data.final_price" />
                </b-col>
              </b-row>
            </div>
            ...
            <div id="pills-grid-view" class="tab-pane fade" role="tabpanel" aria-labelledby="grid-view-tab">
              <b-row cols="1" cols-md="2" :cols-lg="row">
                <b-col v-for="data in filterWiseProducts" :key="data">
                  <CardShopGridView :thumbnail="data.thumbnail" :is-sale="data.is_sale" :is-new="data.is_new" :product-name="data.product_name" :price="data.price" :final-price="data.final_price" />
                </b-col>
              </b-row>
            </div>
            <!-- Three Grid view -->
            <div id="pills-grid-three-view-tab" class="tab-pane fade active show" role="tabpanel" aria-labelledby="grid-three-view-tab">
