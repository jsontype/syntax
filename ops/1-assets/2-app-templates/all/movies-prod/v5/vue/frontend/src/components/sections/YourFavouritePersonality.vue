<template>
  <!-- your favourite personality start -->
  <SectionSlider :title="$t(title)" :list="topTen" :slides-per-view="11" class="streamit-block personality-card" margin-padding="px-md-3 px-1 my-4">
    <template #headerLink>
      <router-link :to="{ name: 'landing-page.personality' }" class="text-primary iq-view-all text-decoration-none flex-none">{{ $t('ott_home.view_all') }}</router-link>
    </template>
    <template #default="{ data }">
      <personality-card :category="data.category" :image="data.image" :title="data.title" :category-link="{ path: '/' }" :link="{ name: 'landing-page.cast-detail' }"></personality-card>
    </template>
  </SectionSlider>
  <!-- your favourite personality end -->
</template>
...
const title = ref('ott_home.personality');
const topTen = ref([
  {
