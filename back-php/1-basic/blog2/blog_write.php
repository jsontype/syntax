...
<form action="_blog_write.php" method="post" enctype="multipart/form-data">

  <!-- ★ 블로그 ID -->
  <input type="hidden" name="blogid" value="blog2">

  <!-- userid : 세션아이디입력 -->
  <input type="hidden" name="id" value="<?=$_SESSION['id']?>">
  <input type="hidden" name="nickname" value="<?=$_SESSION['nickname']?>">
  <!-- <input type="hidden" name="username" value="<'?=$_SESSION['username']"> // username은 세션있으나 블로그에서 사용안함 -->
  ...
  * 내용 : <textarea class="form-control" cols="60" rows="10" name="content" placeholder="내용" minlength="1"></textarea><br>
  ...
  <button type="button" onclick="location.href='blog.php'">뒤로가기</button>
</form>
