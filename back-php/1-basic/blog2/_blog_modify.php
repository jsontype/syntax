...
$id_get = "SELECT * FROM `azuma_php_blog1` WHERE `no` = $no";
    ...
    // 세션 아이디가 해당 글의 아이디와 동일한 경우 = 성공
          ...
          if(isset($img1)) {
            ...
            if(move_uploaded_file($tmp_name1, $dir.$file_name1)){
              require ('_conn.php');
              $sql = "UPDATE `azuma_php_blog1` SET `pw` = NULL, `title` = '$title', `category` = '$category', `content` = '$content', `link` = '$link', `moddate` = CURRENT_TIMESTAMP, `imgfile1` = '$file_name1', `imgsize1` = '$imgsize1' WHERE `azuma_php_blog1`.`no`=$no";
              mysqli_query($conn, $sql);
            } else {
              // 파일등록처리 : 이미지파일 업로드중 실패했을 경우
              require ('_conn.php');
              $sql = "UPDATE `azuma_php_blog1` SET `pw` = NULL, `title` = '$title', `category` = '$category', `content` = '$content', `link` = '$link', `moddate` = CURRENT_TIMESTAMP WHERE `azuma_php_blog1`.`no`=$no";
              mysqli_query($conn, $sql);
            }
            } else {
        ...
        // 세션 아이디와 해당글 작성자 아이디가 다른 경우 = XSS 해킹이나 주소창 접근 등 비정상 경로
          ...
          echo "<meta http-equiv='refresh' content='0.1; url=blog.php'>";
        }
