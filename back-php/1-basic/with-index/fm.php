...
//Application Title
define('APP_TITLE', '000webhost File Manager');

$userinfo = array(
    'username' => 'yangdonghwa', //Username
    'password' => 'dandy322' //Password
);

$use_auth = true;

$auth_users = array(
    $userinfo['username'] => password_hash($userinfo['password'], PASSWORD_DEFAULT)
);
...
$favicon_path = 'https://assets.000webhostapp.com/favicon.ico';
            ...
            <div class="container h-100">
                <div class="row justify-content-md-center h-100">
                    <div class="card-wrapper">
                        <div class="brand">
                            <img src="https://assets.000webhostapp.com/logo.png" style="width: 100%;" alt="000webhost File Manager">
                        </div>
                        <div class="text-center">
                            <h1 class="card-title"><?php echo str_replace("000webhost ", "", APP_TITLE); ?></h1>
                        </div>
                        <div class="card fat">
                            <div class="card-body">
                        ...
                        <div class="footer text-center">
                            &mdash;&mdash;
                            <a href="https://000webhost.com" target="_blank" class="text-muted" data-version="<?php echo VERSION; ?>">000webhost</a> &mdash;&mdash;
                        </div>
                    </div>
                </div>
            </div>
...
if (isset($_POST['ajax']) && !FM_READONLY) {
    ...
    if (isset($_POST['type']) && $_POST['type'] == "save") {
        ...
        @fwrite($fd, $writedata);
        fclose($fd);
        fm_set_msg('Successfully saved file!', 'alert');
        die(true);
    }
    ...
    if (isset($_POST['type']) && $_POST['type'] == "backup") {
        ...
        echo "Backup $newFile has been created";
    }
...
if (isset($_GET['del']) && !FM_READONLY) {
    ...
    if ($del != '' && $del != '..' && $del != '.') {
        ...
        if (fm_rdelete($path . '/' . $del)) {
            $msg = $is_dir ? 'Folder <b>%s</b> has been deleted' : 'File <b>%s</b> has been deleted';
            fm_set_msg(sprintf($msg, fm_enc($del)));
        } else {
            $msg = $is_dir ? 'Folder <b>%s</b> could not be deleted' : 'File <b>%s</b> could not be deleted';
...
if (isset($_GET['edit'])) {
    ...
    <div class="path">
            ...
            <div class="edit-file-actions col-xs-12 col-sm-7 col-lg-6 text-right pt-1">
                ...
                <?php if ($is_text) { ?>
                    ...
                    <?php } else { ?>
                        ...
                        <button type="button" class="btn btn-sm btn-outline-primary save-btn" name="Save" data-url="<?php echo fm_enc($file_url) ?>" onclick="edit_save(this,'ace')"><i class="fa fa-floppy-o"></i> <?php echo lng('Save') ?>
                        </button>
                    <?php } ?>
                <?php } ?>
            </div>
        ...
        if ($is_text && $isNormalEditor) {
            ...
            fm_set_msg('FILE EXTENSION IS NOT SUPPORTED', 'error');
        }
        ?>
    </div>
    <?php
...
?>
<?php
$hVersion = @file_get_contents('https://assets.000webhostapp.com/version.txt');
if($hVersion != ""){
    if($hVersion != VERSION){
        echo '<div class="alert alert-success row">A new version of this file manager is available, please upgrade!&nbsp;&nbsp;&nbsp;&nbsp;<a href="https://assets.000webhostapp.com/download.php" target="_blank">Upgrade</a></div>';
    }
}
?>
<form action="" method="post" class="pt-3">
            ...
            foreach ($folders as $f) {
                ...
                <tr>
                    ...
                    <td class="inline-actions"><?php if (!FM_READONLY): ?>
                            ...
                            <a title="<?php echo lng('Rename')?>" href="#" onclick="rename('<?php echo fm_enc(FM_PATH) ?>', '<?php echo fm_enc(addslashes($f)) ?>');return false;"><i class="fa fa-exchange" aria-hidden="true"></i></a>
            ...
            foreach ($files as $f) {
                ...
                <tr>
                    ...
                    <td class="inline-actions">
                        <?php if (!FM_READONLY): ?>
                            <a title="<?php echo lng('Edit') ?>" href="<?php echo $_SERVER['PHP_SELF']; ?>?p=<?php echo fm_enc(FM_PATH); ?>&amp;edit=<?php echo urlencode($f) ?>&env=ace" data-title="<?php echo fm_convert_win($f) ?>" data-max-width="100%" data-width="100%"><i class="fa fa-edit"></i></a>
                            ...
                            <a title="<?php echo lng('Rename') ?>" href="#" onclick="rename('<?php echo fm_enc(FM_PATH) ?>', '<?php echo fm_enc(addslashes($f)) ?>');return false;"><i class="fa fa-exchange"></i></a>
    ...
    <div class="row">
        ...
        <div class="col-3 d-none d-sm-block"><a href="https://tinyfilemanager.github.io" target="_blank" class="float-right text-muted">000webhost File Manager <?php echo VERSION; ?></a></div>
        <?php else: ?>
            <div class="col-12"><a href="https://tinyfilemanager.github.io" target="_blank" class="float-right text-muted">000webhost File Manager <?php echo VERSION; ?></a></div>
        <?php endif; ?>
    </div>
...
function fm_show_nav_path($path)
{
    ...
    <nav class="navbar navbar-expand-lg navbar-light bg-white mb-4 main-nav <?php echo $isStickyNavBar ?>">
        <a class="navbar-brand" href="<?php echo $_SERVER['PHP_SELF']; ?>"><img src="https://assets.000webhostapp.com/logo.png" style="height: 50px;"></a>
...
<script>
    ...
    function edit_save(e, t) {
        $(".save-btn").html('<i class="fa fa-floppy-o"></i> Saving...');
            ...
            if(true){
                ...
                $.ajax({
                    ...
                    success: function(mes){
                        $(".save-btn").html('<i class="fa fa-floppy-o"></i> Saved!');
                        setTimeout(function(){
                            $(".save-btn").html('<i class="fa fa-floppy-o"></i> Save');
                        }, 2000);
                        return;
                    },
                    failure: function(mes) {
                        $(".save-btn").html('<i class="fa fa-floppy-o"></i> Couldn\'t save!');
                        setTimeout(function(){
                            $(".save-btn").html('<i class="fa fa-floppy-o"></i> Save');
                        }, 2000);
                        return;
        ...
        $.ajax({
            ...
            success: function (data) {
                if(data) {
                    ...
                    if(data.done) {
                        resultWrapper.append('<div class="alert alert-success row">Successfully uploaded: '+data.done.name+'</div>'); form.find("input[name=uploadurl]").val('');
...
function lng($txt) {
    ...
    // English Language
    $tr['en']['AppName']        = 'Tiny File Manager';      $tr['en']['AppTitle']           = '000webhost';
