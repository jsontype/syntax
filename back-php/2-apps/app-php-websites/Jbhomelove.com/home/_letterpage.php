...
// ▲ 페이지 없을 시 1 자동부여
$start_bbs = ($current_bbs_page-1)*9;
// ▲ ★ 넘기는 개수 수정할 때 맨 끝 숫자 수정
$sqlall_bbs = "SELECT * FROM `azuma_jb_bbs1` where delflag = '0'"; // ★ 블로그 ID
$count_bbs = mysqli_query($conn, $sqlall_bbs)->num_rows;
$end_bbs_page = ceil($count_bbs/9);
// ▲ ★ 넘기는 개수 수정할 때 맨 끝 숫자 수정
$sql_bbs = "SELECT * FROM `azuma_jb_bbs1` where delflag = '0' order by no desc limit $start_bbs, 9"; // ★ 블로그 ID
// ▲ ★ 넘기는 개수 수정할 때 맨 끝 숫자 수정
