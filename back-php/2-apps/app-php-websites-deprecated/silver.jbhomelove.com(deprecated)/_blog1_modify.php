    ...
    // 세션 아이디가 해당 글의 아이디와 동일한 경우 = 성공
          ...
          if(isset($img1)) {
            ...
            if(move_uploaded_file($tmp_name1, $dir.$file_name1)){
              require ('_conn.php');
              $sql = "UPDATE `azuma_d_blog1` SET `title` = '$title', `content` = '$content', `imgfile1` = '$file_name1', `imgsize1` = '$imgsize1' WHERE `azuma_d_blog1`.`no`=$no";
              mysqli_query($conn, $sql);
            } else {
              // 파일등록처리 : 이미지파일 업로드중 실패했을 경우
              require ('_conn.php');
              $sql = "UPDATE `azuma_d_blog1` SET `title` = '$title', `content` = '$content' WHERE `azuma_d_blog1`.`no`=$no";
              mysqli_query($conn, $sql);
            }
            } else {
