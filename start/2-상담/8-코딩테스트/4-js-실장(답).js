// [코딩테스트 실장형식: JS 기초 관련. 함수 채우기형 문제 (답)]
...
function listUndoneTasks02(items) {
  return items.filter((task) => !task.done).map((task) => formatTask01(task))
}
...
function sortTasks07(tasks) {
  ...
  return sorted.map((t) => formatTask01(t))
}
...
function addUniqueTask15(newTask, items) {
  if (!items.some((item) => item.name === newTask.name)) {
    items.push(newTask)
  }
}
...
function uppercaseNames19(items) {
  return items.map((i) => ({ ...i, name: i.name.toUpperCase() }))
}
