...
// 이 연산자는 왼쪽 피연산자가 null이나 undefined일 경우에만 오른쪽 피연산자의 값을 반환한다.
// 그렇지 않으면 왼쪽 피연산자의 값을 반환합니다. 이는 기본 값을 설정할 때 매우 유용하며, || 연산자와 비슷하지만,
// 0, NaN, 빈 문자열과 같이 "falsy"하지만 유효한 값들을 "falsy"로 간주하지 않는다는 차이가 있습니다.
// ?? 연산자 사용
const output = input ?? "기본값";
console.log(output); // 빈 문자열인 ""를 출력 ("", 이 경우 빈 문자열은 유효한 값으로 간주)

// || 논리연산자 : 기본값이 출력되었을 것
const output2 = input || "기본값";
console.log(output2); // "기본값" 출력
