/*
  ...
  1. JS는 인터프리터 언어
    JS는 인터프리터 언어이다.
    인터프리터 언어는 코드를 한줄씩 읽어서 실행한다.
    그래서 코드를 한줄씩 읽어서 실행하는 공간이 필요하다.
    이 공간을 "스택"이라고 한다.
    * JS와는 반대로 Java, Python 같은 언어는 컴파일 언어이다. 컴파일 언어는 코드를 한번에 읽어서 실행한다.

  1. JS가 웹브라우저에서 동작하는 원리
    웹브라우져는 다음과 같은 공간들이 있다.
      Heap : i = { age: 20 }, j = { name: 'kim' } 과 같이 변수를 담아두는 공간이다.
      대기실 : 동기함수는 이곳을 건너뛴다.
        대기시간이 끝나면, que라는 "본" 대기실로 다시 넘겨준다.
        대기실 보내는 코드들 : async function (ajax api 호출요청코드, io, setTimeout 등 비동기 함수들과, 이벤트리스너 함수들)
      Que(ue) : 동기함수는 이곳을 건너뛴다.
        대기실에서 "처리가 완료된" 코드 스택들을 줄을 세워서 Que에 세운 뒤, stack으로 하나씩 올려보내준다.
...
// "힙-큐-스택" 코드로 이해하기 : 아래 코드의 예상 순서는 2, 4, 6일 것 같지만, 2, 6, 4가 된다. 이렇게 js는 코드 위에서 적든, 밑에 적든, 빠른 것부터 실행해준다.
