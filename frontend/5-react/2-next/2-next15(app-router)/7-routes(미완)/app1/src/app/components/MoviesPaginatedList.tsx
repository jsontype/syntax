...
// API 호출 함수
async function getData(sort: string = 'rating', limit: number = 20, page: number = 1): Promise<MoviePaginatedList> {
...
export default function MoviesPaginationList({ searchParams }: Props) {
  // 현재 페이지를 표시하기 위한 상태
  const [currentPage, setCurrentPage] = useState(1)

  const queryClient = useQueryClient()
  const queryKey = ['movies', 'paginatedList', searchParams.sort, searchParams.limit, currentPage]
  ...
  */
  const {
    ...
    queryKey,
    queryFn: () => getData(searchParams.sort, searchParams.limit, currentPage),
  })
